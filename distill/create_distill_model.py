...
def create_teacher_model(weights, device):
    ...
    teacher_model = {"model": model, "stride": stride, "names": names}
...
def infer_teacher(model_info, imgs, student_cls_names):
    ...
    assert sorted(student_cls_names) == sorted(teacher_cls_names), ValueError(
        "class names no match between teach and student"
    )

    model = model_info["model"]
    pred = model(imgs)[0]
    pre_h, pre_w = imgs.shape[-2:]
    output_distill, _output = non_max_suppression(pred, 0.25, 0.45, False, max_det=1000)
