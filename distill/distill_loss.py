...
class ComputeLossDistill:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        tcls, tbox, indices, anchors, _tconf = self.build_targets(p, targets)  # targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = bbox_iou(pbox, tbox[i], xywh=True, CIoU=True)  # iou(prediction, target)
    ...
    def build_targets(self, p, targets):
        ...
        tcls, tbox, indices, anch, tconf = (
            [],
            [],
            [],
            [],
            [],
        )  # tcls表示类别，tbox表示box的坐标（x,y,w,h），indices表示图像索引，anch表示选取的anchor的索引
        ...
        )  # [na,nt] same as .repeat_interleave(nt)
        ...
        for i in range(self.nl):  # 循环3个特征层
            ...
            )  # image_id, anchor_id,与网格坐标grid_x，grid_y
